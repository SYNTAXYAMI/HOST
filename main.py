...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = os.environ.get('SESSION_SECRET', 'nehost_ultra_pro_max_99')
    ...
    # FIX: Added /files/read/<folder> route that the dashboard JS calls
    ...
    def fsave_query(folder):
        name = request.args.get('name', request.json.get('name', ''))
        sub_path = request.args.get('path', request.json.get('path', ''))
    ...
    # FIX: Added /server/command/<folder> route that the dashboard JS calls
    ...
    def server_command(folder):
        ...
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        try:
            result = subprocess.run(cmd, shell=True, cwd=path, capture_output=True, text=True, timeout=10)
